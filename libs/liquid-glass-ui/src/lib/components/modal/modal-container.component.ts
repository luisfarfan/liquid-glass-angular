...
import { LiquidModalConfig } from './modal.types';

/**
 * LiquidModalContainerComponent
 ...
 */
@Component({
  selector: 'lg-modal-container',
  standalone: true,
  imports: [CommonModule, DialogModule, PortalModule, A11yModule],
  template: `
    <div class="lg-modal-wrapper" (click)="_onWrapperClick($event)">
      <div 
        class="lg-modal-container"
        [class]="'el-' + (liquidConfig.elevation || 2)"
        [class]="'lg-modal-anim-' + (liquidConfig.animation || 'cinema')"
...
export class LiquidModalContainerComponent extends CdkDialogContainer {
  ...
  get liquidConfig(): LiquidModalConfig {
    return this._config as LiquidModalConfig;
  }
