...
export class RadioGroupComponent implements ControlValueAccessor, AfterContentInit {
  /** Inputs */
  name = input<string>(`lg-radio-group-${Math.random().toString(36).substring(2, 9)}`);
  ...
  /** Query Children */
  items = contentChildren(RadioButtonComponent, { descendants: true });

  /** CDK Focus Management */
  private _keyManager!: FocusKeyManager<RadioButtonComponent>;

  /** ARIA */
  @HostBinding('attr.role') readonly role = 'radiogroup';
  @HostBinding('class.lg-radio-group') readonly hostClass = true;
