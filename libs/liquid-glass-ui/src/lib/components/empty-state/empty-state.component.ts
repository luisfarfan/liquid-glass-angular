...
export type EmptyStateIcon = (typeof KNOWN_ICONS)[number];

/**
 * Glass Empty State — SDD 16.
 * Estados vacíos con orbe ambiental, icono y copy escalonados.
 */
@Component({
  ...
  template: `
    <section
      class="lg-empty-state"
      role="region"
      [attr.aria-labelledby]="titleElementId()"
    >
      @if (ambientGlow()) {
        <div class="lg-empty-state-orb" aria-hidden="true"></div>
      }

      <div class="lg-empty-state-inner">
        <div class="lg-empty-state-icon-wrap" aria-hidden="true">
          @switch (resolvedIcon()) {
            @case ('search') {
              <svg
                class="lg-empty-state-svg"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                stroke-width="2"
        ...
        <div class="lg-empty-state-stagger-copy">
          <h2 class="lg-empty-state-title" [id]="titleElementId()">
            {{ displayTitle() }}
          </h2>
          @if (hasDescription()) {
            <p class="lg-empty-state-desc">{{ description() }}</p>
          }
          <div class="lg-empty-state-actions">
            <ng-content select="[lgEmptyStateActions]"></ng-content>
          </div>
        </div>
      </div>
    </section>
...
export class EmptyStateComponent {
  ...
  private readonly _autoTitleId = `lg-empty-state-title-${++EmptyStateComponent._idSeq}`;
  ...
  readonly resolvedIcon = computed((): EmptyStateIcon => {
    const v = this.icon();
