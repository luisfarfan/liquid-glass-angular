...
/**
 * GngSparkline
 * A minimalist data visualization component for displaying trends within cards and lists.
 */
@Component({
  selector: 'gng-sparkline',
  standalone: true,
  ...
  styles: [`
    .gng-sparkline {
      display: block;
      overflow: visible;
      filter: drop-shadow(0 0 4px rgba(var(--gng-p-primary-rgb), 0.2));
    }
    .gng-sparkline-path {
...
export class GngSparkline {
  ...
  protected gradientId = `gng-sparkline-grad-${Math.random().toString(36).substring(2, 9)}`;
  ...
  trendColor = computed(() => {
    ...
    if (vals.length < 2) return 'var(--gng-t-primary)';
