...
import { GngRadioGroup } from './radio-group.component';

@Component({
  selector: 'gng-radio-button',
  standalone: true,
  imports: [CommonModule],
  template: `
    <div class="gng-radio-container" [class.label-before]="labelPosition() === 'before'">
      <div class="gng-radio-outer" [attr.tabindex]="-1">
        <div class="gng-radio-inner" [style.background-color]="color()" [class.ignited]="checked()"></div>
      </div>
      <span class="gng-radio-label">
        {{ label() }}
...
export class GngRadioButton implements FocusableOption {
  protected group = inject(GngRadioGroup);
  ...
  color = input<string>('var(--gng-t-primary)');
