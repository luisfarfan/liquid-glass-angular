...
import { GngToastConfig, GngToastRef, GngToastType, GNG_TOAST_DEFAULT_CONFIG, GngToastGlobalConfig } from './toast.types';
import { GngToastContainer } from './toast-container.component';
import { Subject } from 'rxjs';

interface ContainerInstance {
  overlayRef: OverlayRef;
  containerRef: ComponentRef<GngToastContainer>;
}

@Injectable({
  providedIn: 'root'
})
export class GngToastService {
  private _overlay = inject(Overlay);
  private _globalConfig = inject(GNG_TOAST_DEFAULT_CONFIG, { optional: true }) || {} as GngToastGlobalConfig;
   ...
   */
  show(config: GngToastConfig): GngToastRef {
  ...
  /** Success shortcut */
  success(message: string, title?: string, config?: Partial<GngToastConfig>) {
    return this.show({ message, title, type: 'success', ...config });
  }

  /** Error shortcut */
  error(message: string, title?: string, config?: Partial<GngToastConfig>) {
    return this.show({ message, title, type: 'error', ...config });
  }

  /** Info shortcut */
  info(message: string, title?: string, config?: Partial<GngToastConfig>) {
  ...
  private _getOrCreateContainer(positionKey: string, vPos: string, hPos: string): ContainerInstance {
    ...
    const overlayRef = this._overlay.create({
      ...
      panelClass: ['gng-toast-overlay-panel', `gng-toast-pos-${vPos}`]
    });

    const portal = new ComponentPortal(GngToastContainer);
