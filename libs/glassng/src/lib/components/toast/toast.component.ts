...
/**
 * GngToast
 * Premium feedback notification with high-fidelity glass-morphism, haptic support and kinetic progress bar.
 */
@Component({
  selector: 'gng-toast',
  standalone: true,
  ...
  template: `
    <div 
      class="gng-toast-item" 
      [class]="'gng-toast-' + config.type"
      (mouseenter)="_onMouseEnter()"
      (mouseleave)="_onMouseLeave()"
    >
      <div class="gng-toast-item-content">
        <!-- Semantic Icon Area -->
        <div class="gng-toast-icon-area">
        ...
        <!-- Content Area -->
        <div class="gng-toast-text-area">
          <div *ngIf="config.title" class="gng-toast-title">{{ config.title }}</div>
          <div class="gng-toast-message">{{ config.message }}</div>
        </div>

        <!-- Close Button -->
        <button *ngIf="config.dismissible !== false" class="gng-toast-close" (click)="_onDismiss()">
...
export class GngToast implements OnInit, OnDestroy {
  @Input({ required: true }) config!: GngToastConfig;
