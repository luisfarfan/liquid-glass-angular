...
/**
 * Valid animation profiles for the Gng Modal
 */
export type GngModalAnimation = 'cinema' | 'glass' | 'zoom' | 'slide' | 'void' | 'none';

/**
 * Configuration for the Gng Modal
 */
export interface GngModalConfig<D = any> extends Omit<DialogConfig<D>, 'container' | 'backdropClass'> {
 ...
 */
export const GNG_MODAL_DATA = new InjectionToken<any>('GNG_MODAL_DATA');

/**
 * Injection Token for global modal configuration
 */
export const GNG_MODAL_DEFAULT_CONFIG = new InjectionToken<GngModalConfig>('GNG_MODAL_DEFAULT_CONFIG');

/**
 * Interface for GngModalRef (wrapper over DialogRef)
 */
export abstract class GngModalRef<R = any> {
