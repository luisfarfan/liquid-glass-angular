...
@Component({
  selector: 'gng-checkbox',
  standalone: true,
  imports: [CommonModule],
  template: `
    <div class="gng-checkbox-box" 
         [class.is-checked]="checked() && !indeterminate()"
         [class.is-indeterminate]="indeterminate()"
         (click)="toggle($event)">
      
      <!-- SVG Icon -->
      <svg class="gng-checkbox-icon" viewBox="0 0 24 24">
        @if (indeterminate()) {
    ...
    <span class="gng-checkbox-label" (click)="toggle($event)">
      {{ label() }}
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => GngCheckbox),
      multi: true
    }
  ]
})
export class GngCheckbox implements ControlValueAccessor {
  /** Inputs & Signals */
