...
import { GngCheckbox } from '../../checkbox/checkbox.component';
import { GngButton } from '../../button/button.component';

export interface GngColumnDef {
  key: string;
  label: string;
  visible: boolean;
...
@Component({
  ...
  template: `
    <gng-glass-card class="bg-glass-card/95 backdrop-blur-3xl border border-glass-border shadow-2xl p-4 w-[250px]">
      ...
      <div class="space-y-1 mb-4 max-h-[300px] overflow-y-auto gng-scroll-thin pr-1">
        <div *ngFor="let col of columns(); trackBy: trackByKey" 
          ...
          <label class="flex items-center gap-3 cursor-pointer flex-1">
            <gng-checkbox [(ngModel)]="col.visible" 
                        [disabled]="!!col.fixed"
                        (ngModelChange)="onToggle()"></gng-checkbox>
      ...
      <div class="flex items-center justify-between pt-2 border-t border-glass-border">
        <button gng-button variant="text" size="sm" (click)="reset()">Reset All</button>
        <button gng-button variant="text" size="sm" (click)="hideAll()">Hide All</button>
      </div>
    </gng-glass-card>
  `,
...
export class GngColumnSelector {
  /** Initial column definitions. */
  @Input({ required: true }) initialColumns: GngColumnDef[] = [];
  ...
  protected columns = signal<GngColumnDef[]>([]);

  ngOnInit() {
    let saved: string[] | null = null;
    if (this.persistenceKey) {
      const data = localStorage.getItem(`gng-table-cols-${this.persistenceKey}`);
      if (data) {
  ...
  private save() {
    ...
    const visibleKeys = this.columns()
      .filter(c => c.visible)
      .map(c => c.key);
    localStorage.setItem(`gng-table-cols-${this.persistenceKey}`, JSON.stringify(visibleKeys));
  }
  ...
  trackByKey(index: number, item: GngColumnDef) {
    return item.key;
  }
}
