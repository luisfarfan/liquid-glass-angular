 ...
 */
@Component({
  selector: 'gng-textarea',
  standalone: true,
  imports: [CommonModule, FormsModule, TextFieldModule],
  template: `
    <div class="gng-textarea-group">
      <div class="gng-textarea-label-container">
        @if (label()) {
          <label 
            [id]="uid + '-label'"
            [class.is-focused]="isFocused()" 
            class="gng-textarea-label"
          >
            {{ label() }}
          </label>
        }
      ...
      @if (error()) {
        <span [id]="uid + '-error'" class="gng-textarea-error-msg">
          {{ error() }}
        </span>
      }
    </div>
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => GngTextarea),
      multi: true
    }
  ],
  host: {
    '[class.gng-textarea]': 'true'
  }
})
export class GngTextarea implements ControlValueAccessor {
  /** Inputs */
  ...
  readonly uid = `gng-txt-${Math.random().toString(36).substring(2, 9)}`;
