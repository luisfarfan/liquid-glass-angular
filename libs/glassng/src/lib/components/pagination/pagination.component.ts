...
import type { GngPageEvent } from './pagination.types';

/**
 * GngPagination
 * Premium data navigation with glass-morphism controls, smooth transitions and accessible range display.
 */
@Component({
  selector: 'gng-pagination',
  standalone: true,
  imports: [CommonModule, FormsModule, GngButton, GngSelect, GngSelectOption],
  template: `
    <nav class="gng-pagination" [attr.aria-label]="ariaLabel()">
      <div class="gng-pagination-meta">
        <span class="gng-pagination-range">{{ rangeLabel() }}</span>
      </div>

      <div class="gng-pagination-controls">
        @if (showPageSizeSelector()) {
          <div class="gng-pagination-select">
            <gng-select
              [label]="pageSizeLabel()"
            ...
            >
              @for (opt of pageSizeOptions(); track opt) {
                <gng-option [value]="opt" [label]="pageSizeOptionLabel(opt)"></gng-option>
              }
            </gng-select>
          </div>
        }
        ...
        >
          ...
          @for (slot of displayedPageSlots(); track $index) {
            @if (slot === 'ellipsis') {
              <span class="gng-pagination-ellipsis" aria-hidden="true">···</span>
            } @else {
              <button
                gng-button
                variant="ghost"
...
export class GngPagination {
  ...
  readonly pageChange = output<GngPageEvent>();
  ...
  onPagesKeydown(event: KeyboardEvent): void {
    ...
    const btn = target.closest('button.gng-pagination-page-btn');
    const host = event.currentTarget;
    if (!(host instanceof HTMLElement) || !btn || !host.contains(btn)) return;

    const buttons = Array.from(host.querySelectorAll('button.gng-pagination-page-btn')) as HTMLButtonElement[];
