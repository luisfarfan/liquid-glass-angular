...
/**
 * GngProgressBar 
 * Premium loading bar with glass-morphism, indeterminate animations and haptic feedback.
 */
@Component({
  selector: 'gng-progress-bar',
  standalone: true,
  imports: [CommonModule],
  template: `
    <div class="gng-progress-container" 
      ...
      @if (mode() === 'buffer') {
        <div class="gng-progress-buffer" [style.width.%]="buffer()"></div>
      }

      <!-- Main Progress Fill -->
      <div class="gng-progress-fill" 
           [style.width.%]="mode() === 'indeterminate' ? 100 : value()">
        
        <!-- Refraction / Specular Highlight -->
        <div class="gng-progress-refraction"></div>
        
        <!-- Leading Edge Glow -->
        @if (mode() !== 'indeterminate' && value() > 0 && value() < 100) {
          <div class="gng-progress-glow"></div>
        }
      </div>

      <!-- Indeterminate / Query Overlay -->
      @if (mode() === 'indeterminate' || mode() === 'query') {
        <div class="gng-progress-indeterminate-bar" [class.is-query]="mode() === 'query'"></div>
      }
    </div>
  `,
