...
export interface GngFileWithPreview {
  file: File;
  previewUrl?: string;
  name: string;
  size: number;
...
/**
 * GngFileUpload
 * Premium drag-and-drop file uploader with glass-morphism aesthetics, 
 * real-time previews and kinetic feedback.
 */
@Component({
  selector: 'gng-file-upload',
  standalone: true,
  imports: [CommonModule],
  template: `
    <div 
      class="gng-file-upload-container"
    ...
    >
      ...
      <div class="gng-file-upload-content">
        <div class="gng-file-upload-icon-wrapper">
          <i class="ri-upload-cloud-2-line"></i>
        </div>
        <div class="gng-file-upload-text">
          <p class="gng-file-upload-label">{{ label() }}</p>
          <p class="gng-file-upload-hint">{{ hint() }}</p>
        </div>
      </div>

      <!-- Optional Glow Effect -->
      <div class="gng-file-upload-glow"></div>
    </div>

    @if (showPreview() && files().length > 0) {
      <div class="gng-file-preview-list">
        @for (item of files(); track item.file.name; let i = $index) {
          <div class="gng-file-preview-card animate-in fade-in zoom-in duration-300">
            <div class="gng-file-preview-thumb">
              @if (item.previewUrl) {
            ...
            <div class="gng-file-preview-info">
              <span class="gng-file-preview-name">{{ item.name }}</span>
              <span class="gng-file-preview-size">{{ formatSize(item.size) }}</span>
            </div>
            <button 
              type="button" 
              class="gng-file-preview-remove" 
...
export class GngFileUpload {
  ...
  protected readonly files = signal<GngFileWithPreview[]>([]);
  ...
  private handleFiles(fileList: FileList) {
    const newFiles: GngFileWithPreview[] = [];
