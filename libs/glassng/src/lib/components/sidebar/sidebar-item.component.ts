...
import { GngSidebarItemModel, GngSidebarService } from './sidebar.service';
import { Subject, takeUntil } from 'rxjs';
import { filter } from 'rxjs/operators';

/**
 * GngSidebarItem
 * Individual navigation node within the sidebar. Supports nesting and auto-indicator tracking.
 */
@Component({
  selector: 'gng-sidebar-item',
  standalone: true,
  imports: [RouterLink, RouterLinkActive],
  template: `
    <!-- MAIN ITEM BUTTON -->
    <a
      class="gng-sidebar-item"
      ...
      [attr.tabindex]="0"
    >
      <div class="gng-sidebar-item-icon">
        <ng-content select="[icon]"></ng-content>
      </div>
      
      <span class="gng-sidebar-item-label">
        <ng-content></ng-content>
      </span>

      @if (subItems()) {
        <!-- Expand Icon for Parents -->
        <i class="ri-arrow-right-s-line gng-sidebar-expand-icon" [class.is-expanded]="isExpanded()"></i>
      }

      @if (badge() && !service.isCollapsed()) {
        <!-- Optional Badge -->
        <div class="gng-sidebar-badge px-1.5 py-0.5 rounded-full bg-primary/20 text-primary text-[0.625rem] font-bold">
          {{ badge() }}
        </div>
      }
    </a>
    ...
    @if (subItems()) {
      ...
      <div
        class="gng-sidebar-nested-container"
        [class.is-expanded]="isExpanded()"
        (transitionend)="onTransitionEnd($event)"
      >
        <div class="gng-sidebar-nested-wrapper">
          <ng-content select="gng-sidebar-item"></ng-content>
        </div>
      </div>
    }
  `,
...
export class GngSidebarItem implements AfterViewInit, AfterContentInit, OnDestroy, GngSidebarItemModel {
  ...
  private readonly nestedItems = contentChildren(GngSidebarItem, { descendants: true });
  ...
  ngAfterContentInit(): void {
    ...
    this.isNested.set(host.parentElement?.classList.contains('gng-sidebar-nested-container') || 
                   host.parentElement?.classList.contains('gng-sidebar-nested-wrapper'));
  }
  ...
  public isEffectivelyCollapsed(): boolean {
    ...
    while (current && !current.classList.contains('gng-sidebar-content')) {
      if (current.classList.contains('gng-sidebar-nested-container')) {
  ...
  public getIndicatorAnchor(): HTMLElement | null {
    const host = this.elementRef.nativeElement;
    const selfAnchor = host.querySelector(':scope > a.gng-sidebar-item') as HTMLElement | null;
    
    if (this.service.isCollapsed() || this.isEffectivelyCollapsed()) {
      let parent = host.parentElement as HTMLElement | null;
      while (parent && !parent.classList.contains('gng-sidebar-content')) {
        if (parent.tagName.toLowerCase() === 'gng-sidebar-item') {
          return parent.querySelector(':scope > a.gng-sidebar-item') as HTMLElement | null;
        }
