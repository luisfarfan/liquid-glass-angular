...
import { GngButton, GngInput, GngToggle, GngCheckbox } from 'glassng';

@Component({
  selector: 'pg-a11y-page',
  standalone: true,
  imports: [CommonModule, GngButton, GngInput, GngToggle, GngCheckbox],
  ...
  template: `
    ...
    <section class="space-y-6">
      <div class="flex items-center gap-2 mb-4">
        <i class="ri-shield-user-line text-[var(--gng-t-primary)]"></i>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="p-4 rounded-xl bg-glass border border-glass-border space-y-3">
          ...
          <button gng-button [disabled]="true" variant="primary" size="sm" class="w-full">Disabled State</button>
        </div>
        <div class="p-4 rounded-xl bg-glass border border-glass-border space-y-3">
          <p class="text-[10px] font-bold opacity-30 uppercase tracking-widest">A11y Input</p>
          <gng-input [disabled]="true" placeholder="No Editable" size="sm"></gng-input>
        </div>
        <div class="p-4 rounded-xl bg-glass border border-glass-border space-y-3 flex flex-col justify-between">
          ...
          <gng-toggle [checked]="true" labelPosition="before">Status: Active</gng-toggle>
        </div>
        <div class="p-4 rounded-xl bg-glass border border-glass-border space-y-3 flex flex-col justify-between">
          <p class="text-[10px] font-bold opacity-30 uppercase tracking-widest">A11y Checkbox (Before)</p>
          <gng-checkbox [checked]="true" labelPosition="before">I Agree</gng-checkbox>
        </div>
      </div>
